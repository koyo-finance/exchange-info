 ...
 */
export async function getBlocksFromTimestamps(timestamps: number[], bobaBlockClient: ApolloClient<NormalizedCacheObject>, skipCount = 500) {
	if (timestamps?.length === 0) {
		return [];
	}
	const fetchedData: any = await splitQuery(GET_BLOCKS, bobaBlockClient, [], timestamps, skipCount);
